...
|-----------------------------|------|
| `max_steps` | 計画ステップ `max_steps` に到達したとき（既定モードでは 1 回） |
| `late_gap_danger` | 計画の約 **85%** 以降で `Gap` が要注意域に入り、一回だけ判断を促す |
| `flag_continue_toggled` | `continue_rule_holds` が `はい/いいえ` で切り替わったとき |
| `flag_gap_toggled` | `gap_danger` が `はい/いいえ` で切り替わったとき |
| `flag_both_toggled` | 上記 2 つが同時に切り替わったとき |

判断待ち中は `can_advance=false` となり、手動・自動再生とも停止する。

### 3.2 既定モード（`legacy_decision == false`）

判断イベントは「最終判断」ではなく、状態変化に応じた**都度判断**として扱う。

- **続行（`continue`）**  
  - 即終了しない。`running` のまま進行。  
  - `Gap` の要注意しきい値を `+0.1` 緩和し、`0.3` までは `gap_danger=false` とする（`gap_danger_threshold=0.3`）。
  - `max_steps` 到達時の判断は 1 回消化済みとみなす。
- **中止（`stop` / 互換 `llm_stop`）**  
  - その場で `outcome=avoided` として終了。  
  - 客観リスクの「人テーマ平均」に相当する入力（`fatigue`, `attention_loss`）をそれぞれ `0.1` 下げる。  
  - LLM は終了判断の主体ではなく、**中止後のサポート文生成**に限定する。
- **進行上限** … シミュレータ全体のステップ上限は **`MAX_STEPS`（70）**。`can_advance=false` で停止。

```mermaid
flowchart TD
  T[判断トリガ発火<br/>max_steps / flag切替]
  T --> C[判断モーダルで選択]
  C -->|続行| D[running継続]
  D --> E[Gap要注意しきい値を0.3へ緩和]
  C -->|中止| H[outcome: avoided<br/>即終了]
```
...
従来どおり、**計画ステップ到達時の判断だけ**が有効で、選択後の扱いも旧仕様に従う。

- **`continue`** … `gap_danger` または `R_obj ≥ 0.42` で `accident` / `cleared` を分岐。
- **`stop`（互換 `llm_stop`）** … `avoided` で即終了。
...
| デモ説明（静的） | `apps/demo_web/static/guide/`（`llm-role.html` を含む各 HTML と `legacy.html`）、`static/css/demo-guide.css` |
